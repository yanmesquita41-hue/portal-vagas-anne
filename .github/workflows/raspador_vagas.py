...
# Portais corporativos oficiais e confiáveis das indústrias da região (Campinas, Jundiaí, SJC, etc.)
PORTAIS_OFICIAIS = [
...
CARGOS_ALVO = ["PCP", "Supply Chain", "Logística", "Planejador de Produção", "Demand Planner", "S&OP"]
...
def buscar_vagas_reais_oficiais():
    vagas_coletadas = []
    ...
    print("Iniciando varredura estrita apenas em portais oficiais (últimos 7 dias)...")
    
    for portal in PORTAIS_OFICIAIS:
        for cargo in CARGOS_ALVO:
            # Monta uma query restrita obrigatoriamente ao site oficial da empresa e à última semana
            query_estrita = f"site:{portal['dominio']} {cargo}"
            ...
            try:
                ...
                if res.status_code == 200:
                    for item in res.json().get("jobs_results", []):
                        ...
                        # Procura o link de inscrição que pertença ao domínio oficial ou seja seguro
                        ...
                        # Se não achar o link exato no domínio, pega o primeiro link externo válido disponível
                        if not link and apply_opts:
                            candidate_link = apply_opts[0].get("link", "")
                            if candidate_link and "google.com" not in candidate_link:
                                link = candidate_link
                                
                        # Se ainda assim não tiver um link real, descarta para evitar lixo no portal
                        if not link:
                            continue
                ...
                print(f"Erro ao buscar {cargo} na {portal['empresa']}: {e}")
...
def salvar_no_supabase(vagas):
    ...
    print(f"Salvando {len(vagas)} vagas reais e oficiais no Supabase...")
    for vaga in vagas:
        try:
...
if __name__ == "__main__":
    vagas = buscar_vagas_reais_oficiais()
    print(f"Total coletado: {len(vagas)}")
    salvar_no_supabase(vagas)
    print("Varredura de vagas oficiais finalizada com sucesso!")
