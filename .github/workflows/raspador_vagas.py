import os
import requests
from urllib.parse import quote_plus
from supabase import create_client, Client

SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://lqgkytfaaisubgemgved.supabase.co")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "sb_publishable_XU_qqEJD90xA02LKWC1Xhg_Aj0xQ...")
SERPAPI_KEY = "ea4f413bd1cbe50410cb2d7ccca035e2a74781e45f77b5c3e649e9152d12aecb"

supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

def calcular_match_score(descricao_vaga, titulo_vaga):
    score = 50
    texto_completo = f"{titulo_vaga} {descricao_vaga}".lower()
    tags = []
    for kw in ["sap", "s&op", "mrp", "lean manufacturing", "power bi", "excel", "kaizen"]:
        if kw in texto_completo:
            score += 10
            tags.append(kw.upper())
    return min(score, 100), tags if tags else ["Supply Chain", "PCP"]

def limpar_tabela():
    print("Limpando registros antigos do Supabase...")
    try:
        supabase.table("vagas").delete().neq("id", 0).execute()
        print("Tabela limpa com sucesso.")
    except Exception as e:
        print(f"Erro ao limpar tabela: {e}")

def buscar_vagas_eficiente():
    vagas_coletadas = []
    
    if not SERPAPI_KEY:
        print("Erro: SERPAPI_KEY não configurada.")
        return vagas_coletadas

    # Termo unificado e abrangente para gastar apenas 1 crédito por execução
    termo_busca = "PCP Supply Chain Logistica Sao Paulo"
    url = f"https://serpapi.com/search.json?engine=google_jobs&q={quote_plus(termo_busca)}&hl=pt-BR&api_key={SERPAPI_KEY}"
    
    print(f"Consultando SerpApi (Gasto: 1 crédito)...")
    try:
        res = requests.get(url, timeout=20)
        if res.status_code == 200:
            resultados = res.json().get("jobs_results", [])
            print(f"-> Sucesso! Encontradas {len(resultados)} vagas brutas.")
            
            for item in resultados:
                titulo = item.get("title", "")
                empresa = item.get("company_name", "Indústria / Empresa")
                local = item.get("location", "São Paulo - SP")
                descricao = item.get("description", "Descrição oficial coletada do Google Jobs.")
                
                apply_opts = item.get("apply_options", [])
                link = None
                
                for opt in apply_opts:
                    candidate_link = opt.get("link", "")
                    if candidate_link and "google.com" not in candidate_link:
                        link = candidate_link
                        break
                
                if not link and apply_opts:
                    link = apply_opts[0].get("link")
                    
                if not link or "google.com/search" in link:
                    link = f"https://www.google.com/search?q={quote_plus(titulo + ' ' + empresa)}"
                    
                score, tags = calcular_match_score(descricao, titulo)
                
                vagas_coletadas.append({
                    "titulo": titulo,
                    "empresa": str(empresa).capitalize(),
                    "cidade": local,
                    "match_score": score,
                    "tags": tags,
                    "link_da_vaga": link,
                    "descricao": descricao
                })
        else:
            print(f"Erro na API da SerpApi: {res.status_code}")
    except Exception as e:
        print(f"Erro de conexão: {e}")

    # Remove duplicatas
    vagas_unicas = {v['titulo'] + v['empresa']: v for v in vagas_coletadas}.values()
    return list(vagas_unicas)

def salvar_no_supabase(vagas):
    if not vagas:
        print("Nenhuma vaga encontrada para salvar.")
        return

    limpar_tabela()

    print(f"Salvando {len(vagas)} vagas no Supabase...")
    for vaga in vagas:
        try:
            supabase.table("vagas").insert(vaga).execute()
        except Exception as e:
            print(f"Erro ao inserir vaga: {e}")

if __name__ == "__main__":
    vagas = buscar_vagas_eficiente()
    print(f"Total processado: {len(vagas)}")
    salvar_no_supabase(vagas)
    print("Processo finalizado com sucesso!")
